...
Для работы с профилем пользователя используются следующие таблицы: 
...
| Операция     | Таблицы                                           | Расчет                           | Результат     | Запись | Чтение |
|--------------|---------------------------------------------------|----------------------------------|---------------|--------|--------|
| Пользователи | users, user_info, user_photo, user_filters, files | 75 млн. * ~2314 byte             | <= 165.5 ГБ   | 1k     | 20k    |
| Свайпы       | user_filters, reactions, show_history, feed       | 1.6 млрд. * 365 * (33 + 36) byte | 36.65 ТБ      | 18.5k  | 9.25k  |
| Метчи        | reactions, matches                                | 30 млн. * 33 byte * 365          | 344.6 ГБ      | 0.35k  | 0.35   |
| Сообщения    | chat_users, messages                              | 100 млн. * 297 byte * 365        | 9.86 ТБ       | 1.16k  | 2.32k  |
| Фотографии   | user_photo, files                                 | 75 млн * 2МБ                     | 143.05 ТБ     | 1k     | 20k    |
| Сессии       | users, sessionHash                                | 10 млн * 676 byte                | 6.6 ГБ        | 1k     | 20k    |
| **Итого**    |                                                   |                                  | **190.05 TB** |        |        |

![схема_базы_данных](images/bd_scheme_relations.png)

## 6.  Физическая схема базы данных

### Выбор СУБД

Для реализации нашего API будет достаточно использование PostgreSQL и Redis.
...
| СУБД       | Применение                             |
|------------|----------------------------------------|
| PostgreSQL | Хранение долговременных данных         |
| Redis      | Хранение сессий активных пользователей |

### Хранение фотографий:
Фотографии хранятся во всех дата-центрам с реализацией распределенного хранилища по протоколу S3.
Каждая фотография должна храниться хотя бы в двух точках.
...
В каждом дата-центре выделяем ресурсы для 50 шардов.
Схемы баз данных для 1 шарда будет такой:
```
tinder_bd_shard_1: {
    bucket_0
    ...
...
В качестве технологии резервного копирования выбираем инструмент WAL-E для инкрементного резервного копирования.
Этот инструмент позволяют копировать только изменения, что существенно сокращает время и объем резервного копирования.
...
| Объем данных совокупно |
|:----------------------:|
|     2.2 пета байт      |

### Избыточность
Для реализации быстрого доступа к данным конкретного пользователя нужно предусмотреть избыточность данных для некоторых таблиц.

Для таблиц, связанных с конкретным пользователем избыточность не нужна, все данные пользователя лежат в одном шарде, включая информацию о файлах с фотографиями.

### Таблица дублирования данных в разных шардах
| Таблица    | Столбцы или поля, по которым определяется шард при дублировании | Пояснение                                                                                                                                                           |
|------------|-----------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| reactions  | email_hash_from, email_hash_to                                  | реакции пользователей должны быть определены для обоих пользователей этого взаимодействия                                                                           |
| matches    | email_hash_from, email_hash_to                                  | метчи должны храниться для обоих пользователей, чтобы для каждого пользователя можно было найти его метчи в том же шарде, где лежит информация об этом пользователе |
| chats      | email_hash пользователей у которых создается чат                | чат создается для двух пользователей в обоих шардах                                                                                                                 |
| chat_users | email_hash пользователей у которых создается чат                | чат создается для двух пользователей в обоих шардах                                                                                                                 |
| messages   | email_hash пользователей у которых уже есть чат                 | сообщение дублируются в каждом шарде                                                                                                                                |

### Схема работы с СУБД на примере 1 ДЦ
![img.png](images/bd_scheme.png)
...
##### Шаг 3: Выбор шарда
- Случайным образом выберите 1 из шардов для поиска подходящих пользователей 
...
##### Шаг 5: Ранжирование по сходству информации
...
##### Шаг 6: Исключение уже показанных и матчей
...
- Вставьте получившийся результат в конец списка в таблицу `feed` для пользователя (A)

##### Шаг 10: Запись рекомендаций в show_history
- Добавьте выбранных кандидатов в таблицу `show_history` для пользователя (A), чтобы при последующем построении ленты, пользователи не повторялись.

##### Шаг 11: Переход к следующему шарду
- Если лента не заполнилась до нужного размера перейдите к 3 шагу.

##### Шаг 12: Удаление рекомендаций после выдачи и начало формирования новых
-  При запросе ленты пользователя верните список и очистите его в базе данных. Начните формировать новый список. С клиентской стороны зафиксируйте выданный список и используйте его, пока он не закончится.
...
### Точка входа в дата-центр и путь запроса до бекенда

Наличие нескольких дата-центров обеспечивает страховку при падении какого-либо от них, так как запросы будут посылаться на другой.
Как будет распределяться трафик? При падении дата-центра его IP-адрес убирается из DNS-сервера. Последующие запросы будут распределяться в ближайшие дата-центры по GeoDNS.

При входе в ДЦ стоит L3 балансировка, обеспечивающая надежность на уровне 1 дата-центра.
Резервируется N+1 балансировщик, способный обрабатывать входящий трафик. Упавший балансировщик снимается с нагрузки.
Внутренние сервисы создаются в нескольких экземплярах. Это значит, что у Composer, Service Discovery и Service Mesh будет страховка при падении одного из экземпляров.

### Контроль работы приложения и новых версий

Для контроля работы приложения проверяются метрики, по которым строятся графики и отправляются алерты.
Это помогает видеть проблемы в коде и вовремя предотвращать потери.
Pprof помогает выявлять утечки ресурсов.

Canary deploy: постепенная раскатка новых версий сервисов в Kubernetes и возможность откатить релиз обеспечивают надежность системы при выкатке новых версий.
Liveness и readiness пробы в Kubernetes

### Резервирование ресурсов, репликации и копии данных
Мы рассчитали пиковый трафик и объемы хранения данных, поэтому мы можем предсказать, сколько нам нужно ресурсов и памяти для безотказной работы системы.

Для повышения доступности сервисов проводится репликация баз данных, которая является горячей резервной копией БД и обеспечивает надежность сервиса.
При потере всех данных БД, если сервера сгорели, например, их можно восстановить из отдельной резервной копии.

### Надежность на уровне кода

1. Установка лимита на количество запросов для защиты от DDOS-атак
2. Проверки на поведение пользователя-бота, деградация ответов для ботов.
3. Повторные запросы на клиентской стороне кода.
4. Флаги деградации при высоких нагрузках. Например, отдача неготовой ленты.
5. Асинхронная запись в БД.
6. Политика отдельных запросов. Если один запрос не прошел, то другие могут пройти. Например, пользователь в ленте загрузился без описания, но фотографии загрузились.
7. Prefetch ленты и работа в режиме офлайн/при недоступности сервиса с запоминанием действий пользователя.
...
| Сервис      | Пиковый RPS | CPU | RAM    | Network      |
|-------------|-------------|-----|--------|--------------| 
| Авторизация | 18.6k       | 100 | 20  ГБ | 6.8 Мбит/сек |
| Метчи       | 19k         | 100 | 20  ГБ | 3   Мбит/сек |
| Мессенджер  | 1.16k       | 20  | 8   ГБ | 10  Мбит/сек |
| PostgreSQL  | 79k         | 800 | 160 ГБ | 20  Мбит/сек |
| Nginx       | 23k         | 460 | 48  ГБ | 25  Мбит/сек |
| FileServer  | 20k         | 400 | 160 ГБ | 600 Гбит/сек |

 Данные в таблице рассчитаны по пиковым нагрузкам, которые были посчитаны раннее, и по средним размерам контрактов (посчитано в логической схеме БД)

### Конфигурация серверов
| Сервис      | Хостинг     | Конфигурация                                       | Ядра | Количество | Стоимость 1 шт., € | Амортизация на 5 лет, € |
|-------------|-------------|----------------------------------------------------|------|------------|--------------------|-------------------------|
| Авторизация | собственный | 1xIntel Xeon E-2378/2x8ГБ 3200MHz DDR4/1xNVMe256Gb | 8    | 16         | 1900               | 32                      |
| Метчи       | собственный | 1xIntel Xeon E-2378/2x8ГБ 3200MHz DDR4/1xNVMe256Gb | 8    | 16         | 1900               | 32                      |
| Мессенджер  | собственный | 1xIntel Xeon E-2314/1x8ГБ 3200MHz DDR4/1xNVMe256Gb | 4    | 16         | 1000               | 17                      |
| PostgreSQL  | собственный | 1xAMD EPYC 7443P/2x8ГБ 3200MHz DDR4/4xNVMe12T      | 24   | 48         | 6500               | 109                     |
| Nginx       | собственный | 2x6338/4x8GB/2xNVMe4T/2x25Gb/s                     | 64   | 16         | 8000               | 134                     |
| FileServer  | собственный | 1xAMD EPYC 9754/4x8ГБ 4200MHz  DDR4/10xHDD20TB     | 128  | 16         | 20000              | 334                     |

[Источник расчетов цены](https://lg.ruvds.com/)